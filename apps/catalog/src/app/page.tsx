...
import { categories, products, badgeColor, formatPrice } from "./components/CatalogData";
...
const skinTypes = ["All Skin", "Dry Skin", "Oily Skin"];
...
export default function CatalogPage() {
  ...
  const [skinFilter, setSkinFilter] = useState("All Skin");
      ...
      <section className="py-16 sm:py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
            {categories.map((cat) => (
              <button key={cat.name} onClick={() => { setActiveCategory(cat.name); document.getElementById("products")?.scrollIntoView({ behavior: "smooth" }); }}
      ...
      <section id="products" className="py-16 sm:py-20 bg-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h2 className="text-2xl font-bold text-slate-800 sm:text-3xl">{activeCategory === "All" ? "Semua Produk" : activeCategory}</h2>
              <p className="mt-1 text-slate-500 text-sm">{filtered.length} produk ditemukan</p>
            </div>
            <div className="flex items-center gap-2">
              {activeCategory !== "All" && (
                <button onClick={() => setActiveCategory("All")} className="inline-flex items-center gap-1 rounded-full border border-rose-200 px-3 py-1.5 text-xs font-medium text-rose-600 hover:bg-rose-50">
          ...
          {filterOpen && (
            <div className="mb-8 rounded-2xl border border-rose-100 bg-rose-50/50 p-5 animate-fade-up">
              <div className="grid gap-6 sm:grid-cols-3">
                <div>
                  <div className="text-sm font-medium text-slate-700 mb-2">Skin Type</div>
                  <div className="flex flex-wrap gap-2">
                    {skinTypes.map((t) => (
                      <button key={t} onClick={() => setSkinFilter(t)}
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 text-xs text-slate-500">
                    ...
                    <input type="range" min={0} max={500000} step={50000} value={priceRange[1]}
          ...
          <div className="grid gap-6 grid-cols-2 lg:grid-cols-4">
            {filtered.map((product) => (
              <div key={product.id} className="group rounded-2xl border border-slate-100 bg-white overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-rose-100 hover:-translate-y-1">
                <div className={`relative h-56 bg-gradient-to-br ${product.gradient} flex items-center justify-center overflow-hidden cursor-pointer`}
                ...
                <div className="p-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      {product.originalPrice && <span className="text-xs text-slate-400 line-through ml-2">{formatPrice(product.originalPrice)}</span>}
                    </div>
                  </div>
